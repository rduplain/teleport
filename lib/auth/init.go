...
// to open source users. It creates a less privileged role 'user'
...
func migrateOSSTrustedClusters(ctx context.Context, role types.Role, asrv *Server) (int, error) {
	...
	for _, tc := range tcs {
		...
		tc.SetRoleMap([]types.RoleMapping{{Remote: types.Wildcard, Local: []string{role.GetName()}}})
		tc.SetMetadata(meta)
...
func migrateOSSGithubConns(ctx context.Context, role types.Role, asrv *Server) (int, error) {
	...
	// To do that, create a new role per
