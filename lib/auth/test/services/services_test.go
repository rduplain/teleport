...
func (s *TLSSuite) TestGenerateCerts(c *check.C) {
	...
	hostClient, err := s.server.NewClient(Identity{I: server.BuiltinRole{Username: hostID, Role: teleport.RoleNode}})
	c.Assert(err, check.IsNil)
	...
	user1, userRole, err := test.CreateUserAndRole(s.server.Auth(), "user1", []string{"user1"})
	c.Assert(err, check.IsNil)
	...
	nopClient, err := s.server.NewClient(Nop())
	c.Assert(err, check.IsNil)
	...
	adminClient, err := s.server.NewClient(Admin())
	c.Assert(err, check.IsNil)
